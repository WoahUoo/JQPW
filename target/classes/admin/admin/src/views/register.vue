	...
	<div>
		<div class="container" :style='{"minHeight":"100vh","alignItems":"center","background":"url(http://codegen.caihongy.cn/20240220/a89cf7f79399402f91dc50a779d453a3.jpg)","display":"flex","width":"100%","backgroundSize":"cover","backgroundPosition":"center center","backgroundRepeat":"no-repeat","justifyContent":"center"}'>
			<el-form v-if="pageFlag=='register'" :style='{"padding":"20px","boxShadow":"0 1px 20px rgba(64, 158, 255, .5)","margin":"50px 0","borderRadius":"10px","background":"rgba(0,0,0,.75)","width":"550px","height":"auto"}' ref="rgsForm" class="rgs-form" :model="rgsForm" :rules="rules">
				<div v-if="true" :style='{"width":"100%","margin":"0 0 10px 0","lineHeight":"64px","fontSize":"20px","color":"#fff","textAlign":"center"}' class="title">景区票务销售系统注册</div>
				<el-form-item :style='{"width":"80%","padding":"0","margin":"0 auto 2px","height":"auto"}' class="list-item" v-if="tableName=='shoupiaoyuan'">
					<div v-if="true" :style='{"width":"100%","lineHeight":"44px","fontSize":"14px","position":"relative","color":"#fff"}' class="lable" :class="changeRules('shoupiaoyuanzhanghao')?'required':''">售票员账号：</div>
					<el-input  v-model="ruleForm.shoupiaoyuanzhanghao"  autocomplete="off" placeholder="售票员账号"  type="text"  />
				</el-form-item>
				<el-form-item :style='{"width":"80%","padding":"0","margin":"0 auto 2px","height":"auto"}' class="list-item" v-if="tableName=='shoupiaoyuan'">
				...
				<el-form-item :style='{"width":"80%","padding":"0","margin":"0 auto 2px","height":"auto"}' class="list-item" v-if="tableName=='shoupiaoyuan'">
                    ...
                    <file-upload
                        ...
                        @change="shoupiaoyuantouxiangUploadChange"
                    ></file-upload>
				</el-form-item>
				<el-form-item :style='{"width":"80%","padding":"0","margin":"0 auto 2px","height":"auto"}' class="list-item" v-if="tableName=='shoupiaoyuan'">
                    ...
                    <el-select v-model="ruleForm.xingbie" placeholder="请选择性别" >
                        <el-option
                            v-for="(item,index) in shoupiaoyuanxingbieOptions"
                            v-bind:key="index"
                            :label="item"
                            :value="item">
                        </el-option>
				...
				<el-form-item :style='{"width":"80%","padding":"0","margin":"0 auto 2px","height":"auto"}' class="list-item" v-if="tableName=='shoupiaoyuan'">
					<div v-if="true" :style='{"width":"100%","lineHeight":"44px","fontSize":"14px","position":"relative","color":"#fff"}' class="lable" :class="changeRules('pquestion')?'required':''">密保问题：</div>
					<el-input  v-model="ruleForm.pquestion"  autocomplete="off" placeholder="密保问题"  type="text"  />
				</el-form-item>
				<el-form-item :style='{"width":"80%","padding":"0","margin":"0 auto 2px","height":"auto"}' class="list-item" v-if="tableName=='shoupiaoyuan'">
					<div v-if="true" :style='{"width":"100%","lineHeight":"44px","fontSize":"14px","position":"relative","color":"#fff"}' class="lable" :class="changeRules('panswer')?'required':''">密保答案：</div>
...
export default {
	...
	mounted(){
		...
		if(this.$route.query.pageFlag=='register'){
			...
			if(this.tableName=='shoupiaoyuan'){
				this.ruleForm = {
			...
			if ('shoupiaoyuan' == this.tableName) {
				this.rules.shoupiaoyuanzhanghao = [{ required: true, message: '请输入售票员账号', trigger: 'blur' }]
			}
			if ('shoupiaoyuan' == this.tableName) {
			...
			this.shoupiaoyuanxingbieOptions = "男,女".split(',')
		}
	},
	created() {
	},
	...
	methods: {
        ...
        shoupiaoyuantouxiangUploadChange(fileUrls) {
		...
		login() {
			var url=this.tableName+"/register";
					if((!this.ruleForm.shoupiaoyuanzhanghao) && `shoupiaoyuan` == this.tableName){
						this.$message.error(`售票员账号不能为空`);
						return
					}
					if((!this.ruleForm.mima) && `shoupiaoyuan` == this.tableName){
						this.$message.error(`密码不能为空`);
						return
					}
					if((this.ruleForm.mima!=this.ruleForm.mima2) && `shoupiaoyuan` == this.tableName){
						this.$message.error(`两次密码输入不一致`);
						return
					}
					if((!this.ruleForm.xingming) && `shoupiaoyuan` == this.tableName){
            ...
            if(this.ruleForm.touxiang!=null) {
                this.ruleForm.touxiang = this.ruleForm.touxiang.replace(new RegExp(this.$base.url,"g"),"");
					...
					if(`shoupiaoyuan` == this.tableName && this.ruleForm.shoujihaoma &&(!this.$validate.isMobile(this.ruleForm.shoujihaoma))){
						this.$message.error(`手机号码应输入手机格式`);
						return
					}
					if(`shoupiaoyuan` == this.tableName && this.ruleForm.shenfenzhenghao &&(!this.$validate.checkIdCard(this.ruleForm.shenfenzhenghao))){
