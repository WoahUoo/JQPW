...
export default {
	data() {
      ...
      ruleForm: {
			...
			rules: {
				shoupiaoyuanzhanghao: [
					{ required: true, message: '售票员账号不能为空', trigger: 'blur' },
				],
				mima: [
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `shoupiaoyuan/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.shoupiaoyuanCrossAddOrUpdateFlag = false;
											this.parent.search();
